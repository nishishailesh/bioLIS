...
function start_nchsls()
{
	...
	if($ret==100)
	{
		//echo 'link 100 returned';
		$link=get_link('dfasd345','df54*(&sdf');
	}
	else
	{
		echo 'database/table problem? wrong password? expired password?';
		//header("location:chp/index.php"); 
		exit();
	}
...
///////////////////END of For user setting change////////////////////
...
function main_menu()
{
	...
	<li><a href="#">Report</a>
		<ul>
			...
			<li><a href=print_donation.php>Donation</a>
		</ul>
	<li><a href="#">Quality</a>
		<ul>
...
////reflex
        $sql_reflex='select * from reflex_alert where code=\''.$code.'\' and sample_type=\''.$sample_type.'\'';
        if(!$result_reflex=mysql_query($sql_reflex,$link))
        {
                echo 'check_critical_abnormal_reportable(reflex):'.mysql_error();
                return -1;
        }
        
        while($data_reflex=mysql_fetch_assoc($result_reflex))
        {
                //echo '<pre>';
                //print_r($data_critical);
                //echo '</pre>';        
                if($data_reflex['operator']=='less_than' && $result<$data_reflex['value'])
                {
                        return 12;
                }
                if($data_reflex['operator']=='more_than' && $result>$data_reflex['value'])
                {
                        return 13;
                }
        }
...
function tibc($sample_id)
{
	//echo '<h1>Reached tibc()</h1>';
	$uibc=get_examination_result('UIBC',$sample_id);
	$iron=get_examination_result('IRON',$sample_id);

	
	if($uibc!==FALSE && $iron!==FALSE)  	//they are requested
	{
		//echo '222';

		//strip of ( and ) before testing number
		if($uibc[0]=='(')
		{
			$uibc_striped=substr($uibc,1);
		}
		else
		{
			$uibc_striped=$uibc;
		...
		//if($uibc[strlen((string)$uibc_striped)]==')')
		if(substr($uibc_striped,-1)==')')
		{
			$uibc_striped_last=substr($uibc_striped,0,-1);
		}
		else
		{
			$uibc_striped_last=$uibc_striped;
		}		
		
		if($iron[0]=='(')
		{
			$iron_striped=substr($iron,1);
		}
		else
		{
			$iron_striped=$iron;
		...
		//if($iron[strlen((string)$iron_striped)]==')')
		if(substr($iron_striped,-1)==')')
		{
			$iron_striped_last=substr($iron_striped,0,-1);
		}
		else
		{
			$iron_striped_last=$iron_striped;
		}						
		//echo '<h1>XXX'.$uibc_striped_last.'YYY</h1>';
		
		if 	(is_str_num($uibc_striped_last)!==FALSE && is_str_num($iron_striped_last)!==FALSE)	//strict zero and false may be same
		{
				save_single_examination_code($sample_id,'TIBC',($uibc_striped_last+$iron_striped_last));
		}
	}
}
...
{
	if($acr_code==12 || $acr_code==1 || $acr_code==3 ){$level='less_than';}
	elseif($acr_code==13 || $acr_code==2 || $acr_code==4 ){$level='more_than';}
	else{$level='';}
...
{
	global $reflex;
	if(in_array(get_sample_status($sample_id),$GLOBALS['final_stage'])){echo 'sample_id '.$sample_id.' reached verified/released state.Autoverification not done<br>'; return;}
	...
	foreach ($all_details  as $key=>$value)
	{
		...
		//////////reflax if (2,4,6,12,13)   //cholesterol=13
		if(
			$acr_code==12 || 
			$acr_code==13 ||
			$acr_code==3 ||
		...
		{
			echo 'calculating LDL<br>';
			lipid_profile($value['sample_id']);
		}

		///////////////TIBC
		if($value['code']=='TIBC')			//CHOL and CHOV go hand in hand
		{
			echo 'calculating TIBC<br>';
			tibc($value['sample_id']);
		}
...
class MYPDF_HTML extends TCPDF {
	public $lab_name='NEW CIVIL HOSPITAL SURAT LABORATORY SERVICES';
	public $section_name='Biochemistry Section';
	public $section_address_phone='Near Blood Bank, 2nd Floor. Ext:317,366';
	public $address_phone='MAJURA GATE SURAT PHONE NO.0261-2244456';
...
function print_single_report_html_pdf($pdf, $sample_id,$doctor)
{
...
function print_attachment_A4_html($pdf,$sample_id)
{
	$sql='select * from attachment where sample_id=\''.$sample_id.'\'';
	//echo $sql;
	$link=start_nchsls();
	...
	{
		//echo 'd';
		print_single_attachment_A4_html($pdf,$array);
	}
}

function print_single_attachment_A4_html($pdf,$array)
{
	$filetype=$array['filetype'];
	if($filetype=='jpeg' || $filetype=='jpg' || $filetype=='gif')
	{
		//$image_file=create_file($array['file'],$array['sample_id'],$array['attachment_id'],$array['filetype']);
		//$encoded_image='data:image/png;base64,'.base64_encode($array['file']);
		//return '<img src="'.$encoded_image.'" >';
	 ...
	 * @since 1.1
	 * */		
		//$pdf->Image($image_file, 10, $y,100,100, 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
		$pdf->Image('@'.$array['file'], 10, $y,100,100, 'JEPG', '', 'N', false, 300, '', false, false, 0, false, false, false);
	}
...
function print_single_report_realhtml($sample_id,$doctor)
{
	if(!is_str_interger($sample_id)){return;}
...
*/	
	...
	{
		//echo '<h2 style="page-break-before: always;">xxxx</h2>';
		print_attachment_A4_realhtml($sample_id);
	}	
...
function print_attachment_A4_realhtml($sample_id)
{
	$sql='select * from attachment where sample_id=\''.$sample_id.'\'';
	//echo $sql;
	$link=start_nchsls();
	...
	{
		//echo 'd';
		print_single_attachment_A4_realhtml($array);
	}
}

function print_single_attachment_A4_realhtml($array)
{
	$filetype=$array['filetype'];
	if($filetype=='jpeg' || $filetype=='jpg' || $filetype=='gif')
	{
		$encoded_image='data:image/png;base64,'.base64_encode($array['file']);
		echo '<img src="'.$encoded_image.'" >';
	}
	
	elseif($filetype=='txt')
	{
		echo '<pre>'.$array['file'].'</pre>';
	}
}
	...
	function sample_header_realhtml($sample_id) 
	{
		ob_start();

		$linkk=start_nchsls();
		$sample_data=get_sample($sample_id);	
		if($sample_data['status']=='released'){$class='message';}else{$class='warning';}		
		//print_r($sample_data);
		echo '<table border="1">';
		echo '<tr><td>
					<table>
		...
		echo '<table border="1">
						...
						<tr>							
							<td align="left"><span class=message>Status:</span><span id=cur_status class=\''.$class.'\' >'.$sample_data['status'].'</span> by:'.$sample_data['released_by'].'</td>
							<td align="left">Released at: '.$sample_data['released_at'].'</td>
						</tr>
				</table>';
		$myStr = ob_get_contents();
