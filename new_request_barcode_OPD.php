...
function confirm_next_sample_id_for_OPD()
{
	$sample_id=find_next_sample_id_for_OPD();
	$link=start_nchsls();
...
function insert_required_samples($post)
{
	...
	echo '<div id=display>';
		...
		if($req_tube=='P')
		{
			$plain_sample_id=confirm_next_sample_id_for_OPD();
		...
		{
			$fluoride_sample_id=confirm_next_sample_id_for_OPD();
		...
		if(isset($fluoride_sample_id))
		{
			...
			$sample_array['unit']=$unit;
			$sample_array['location']=$location;
